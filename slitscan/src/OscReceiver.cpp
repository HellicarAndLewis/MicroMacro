...
void OscReceiver::update(){
    ...
    while(receiver.hasWaitingMessages()){
        ...
        if (m.getAddress() == "/mode") {
            string groupIdsString = m.getArgAsString(0);
            RemoteEvent eventArgs(RemoteEvent::SWITCH_MODE, groupIdsString);
            ofNotifyEvent(RemoteEvent::events, eventArgs);
        }
...
void OscReceiver::keyReleased(int key){
    
}


void OscReceiver::mouseMoved(int x, int y){
...
void OscReceiver::gotMessage(ofMessage msg){
    
}


void OscReceiver::dragEvent(ofDragInfo dragInfo){
